context("App 200 onboarding", () => {
    ...
    it("complete questionnaire", () => {
      ...
      cy.get(`[data-cy="let's_go!Button"]`).click().wait('@next');

      cy.get("#P10_QUESTION").should(
        "contain",
        "What type of area do you live in?"
      );
      cy.get(".fabe-answer-pill:first").click().wait('@next');
      cy.get("[data-cy=nextButton]").click().wait('@next');
      ...
      cy.get(".fabe-answer-pill:first").click().wait('@next');
      cy.get('[data-cy=completeButton]').click().wait('@next');

      cy.get('#OnboardingLoading').should('contain','Building a Planet Plan');

      cy.get('#OnboardingRecommendationSummary').should('contain','We Recommend');
      cy.get('[data-cy=start_the_healingButton]').click().wait('@next');
      cy.url().should('contain','200:50:');

      cy.get('.introjs-tooltiptext').should('contain','How will you heal the planet today?');
      cy.get('.introjs-nextbutton').click().wait('@next');
